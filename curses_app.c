...
void count_cds()
{
	...
	mvprintw(ERROR_LINE,0,"Database contains %d titles , with a total of %d trakcs.");
	get_return();
}
...
void list_tracks()
{
	...
	fclose(tracks_fp);
	/* Make a new pad,ensure that even if there is only a single track the PAD is large enough so the later preferesh() is always valid */ 
